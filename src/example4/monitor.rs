...
pub trait MonitorTriggers {
    fn heat_on(arg0: f64) -> ();
    fn heat_off(arg0: f64) -> ();
    fn speed_warning(arg0: f64) -> ();
}
...
impl<T: MonitorTriggers> Monitor<'_, T> {
    ...
    pub fn step(&mut self, input: &MonitorInput) {
        internal_step(input, &mut self.state, &mut self.output);

        if self.output.heat_on_verdict.verdict {
            T::heat_on(self.output.heat_on_verdict.arg0);
        }

        if self.output.heat_off_verdict.verdict {
            T::heat_off(self.output.heat_off_verdict.arg0);
        }

        if self.output.speed_warning_verdict.verdict {
            T::speed_warning(self.output.speed_warning_verdict.arg0);
        }
    }
}
